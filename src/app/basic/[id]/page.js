    ...
    <>
      {basic[get_page_id].isTutorial === false ? (
        ...
        <section className="relative flex flex-col items-center leading-relaxed break-keep">
          <h1 tabIndex={0} className="mb-8 text-3xl text-center lg:text-5xl font-hanna md:text-center md:text-4xl lg:mb-24">
            {basic[get_page_id].title}
          </h1>
          <div className="">
              ...
              return (
                <div className="flex flex-col justify-center mb-6 border-b lg:py-4 lg:gap-8 lg:items-start lg:flex-row" key={element.id}>
                  <div className="mb-2 h-fit lg:flex lg:pt-4 lg:gap-4">
                    <div tabIndex={0} className="flex items-center justify-center w-6 h-6 mx-auto mt-1 mb-3 text-lg text-center rounded-full bg-BM-primary text-BM-white">
                      {element.id}
                    </div>
                    <div className="text-lg font-medium text-center lg:p-0 lg:leading-relaxed lg:w-80 lg:text-left lg:text-xl md:px-8 md:text-sm ">
                      <div tabIndex={0}>{HTMLReactParser(element.desc)}</div>
                    </div>
                  </div>
            ...
            <div className="flex justify-between w-full">
              ...
              <Link className="px-3 py-2 text-sm text-white rounded-full shadow-sm bg-BM-primary" href={get_page_id === "4" ? `/usage/0` : `/basic/${parseInt(get_page_id) + 1}`}>
                다음으로 👉
              </Link>
            </div>
          </div>
